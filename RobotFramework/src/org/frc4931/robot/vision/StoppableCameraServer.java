...
public class StoppableCameraServer {

    private static final int kPort = 1180;
    protected static final byte[] kMagicNumber = { 0x01, 0x00, 0x00, 0x00 };
    ...
    private static StoppableCameraServer server;

    public static StoppableCameraServer getInstance() {
        if (server == null) {
            server = new StoppableCameraServer();
        }
        return server;
    }
    ...
    protected synchronized void startServer() {
        ...
        // BEGIN: ADDED BY FRC 4931
        serverThread.setDaemon(true);
        // END: ADDED BY FRC 4931
        serverThread.start();
    }

    protected synchronized void stopAutomaticCapture() {
        runCapture = false;
        try {
            Thread captureThread = this.captureThread;
     ...
     * without doing any vision processing on the roboRIO. {@link #setImage} shouldn't be called after this is called.
    ...
    protected void capture() {
        System.out.println("Starting automatic image capture");
        Image frame = null;
        try {
     ...
     * 
     * @return true if auto capture was started, or false otherwise
     */
        ...
        while (runServer) {
            try {
                ...
                while (runServer) {
                    ...
                    System.out.println("*** SERVER: image[155] = " + imageArray[155]);
